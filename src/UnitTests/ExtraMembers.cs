...
using DRMWrapperClassGenLib;
#endif

namespace AutoMapper.UnitTests.ExtraMembers
{
#if NET452
    public class When_configuring_extra_members
    {
        ...
        [Fact]
        public void ExtraMembersShouldBeIncludedUseEmit()
        {
            Dictionary<string, Type> typeDefs = new Dictionary<string, Type>
            {
                { "Item1", typeof(string) },
                { "MyInteger", typeof(int) }
            };

            List<DRMWrapperClassGenLib.PropertyDescription> propDescs = GetPropertyDescriptions(typeDefs);
            DRMWrapperClassGenLib.TypeDescription<TypeSafePropBagBase> td = 
                new DRMWrapperClassGenLib.TypeDescription<TypeSafePropBagBase>("Test", propDescs);


            Type DestProxy = DRMWrapperClassGenLib.WrapperGenerator.EmitWrapper(td);

            object generatedObject = Activator.CreateInstance(DestProxy, new object[] { typeDefs});

            Type wrapperType = generatedObject.GetType();

            // Loop over all the generated properties, and assign the values from our XML:
            PropertyInfo[] properties = DestProxy.GetProperties();
            ...
            generatedObject = Activator.CreateInstance(DestProxy, new object[] { typeDefs });
            ...
            {
                cfg
                .CreateMap(typeof(Source), wrapperType)
                .ForMember("NamedValuesWithType", (mc => mc.Ignore()))
                .ForMember("TypeDefs", (mc => mc.Ignore()))
                .ConstructUsing(c => Activator.CreateInstance(DestProxy, new object[] { typeDefs }))
                ;

                //.ForMember("Item1", opt => opt.Condition(srcA => ((Source)srcA).Item1 != "AA"));

                //Func<Destination, bool> cond = (s => "x" != (string)s.GetIt("Item1", typeof(string)));

                //TODO: Ingore the NamedValuesWithType property.
                cfg
                .CreateMap(wrapperType, typeof(Source));
